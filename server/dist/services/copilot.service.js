...
exports.formatGeminiErrorMessage = formatGeminiErrorMessage;
...
class CopilotService {
    getClient(customKey) {
        ...
        if (key && typeof key === 'string' && key.trim().length > 5 && key.trim() !== 'undefined' && key.trim() !== 'null') {
            return new genai_1.GoogleGenAI({ apiKey: key.trim() });
        }
        return null;
    }
    async generateWithGeminiFallback(client, contents, config) {
        const modelsToTry = [
            'gemini-flash-latest',
            'gemini-3.6-flash'
        ];
    ...
    applyLocalSmartDocumentEdit(instruction, html) {
        if (!html || !html.trim())
            return html;
        ...
        if (lower.includes('remove') || lower.includes('delete') || lower.includes('कमी करा') || lower.includes('काढा')) {
            const matchPoint = (numStr, marathiNum, ordinals) => {
        ...
        if (lower.includes('bold') || lower.includes('ठळक') || lower.includes('बोल्ड')) {
            let numToBold = '2';
            let marathiNum = '२';
            if (lower.includes('1') || lower.includes('१') || lower.includes('1st') || lower.includes('first')) {
                numToBold = '1';
                marathiNum = '१';
            }
            else if (lower.includes('2') || lower.includes('२') || lower.includes('2nd') || lower.includes('second')) {
                numToBold = '2';
                marathiNum = '२';
            }
            else if (lower.includes('3') || lower.includes('३') || lower.includes('3rd') || lower.includes('third')) {
                numToBold = '3';
                marathiNum = '३';
            }
            else if (lower.includes('4') || lower.includes('४') || lower.includes('4th') || lower.includes('fourth')) {
                numToBold = '4';
                marathiNum = '४';
            }
            else if (lower.includes('5') || lower.includes('५') || lower.includes('5th') || lower.includes('fifth')) {
                numToBold = '5';
                marathiNum = '५';
            }
        ...
        if (lower.includes('convert') || lower.includes('translate') || lower.includes('english') || lower.includes('marathi') || lower.includes('मराठी')) {
            const targetLang = (lower.includes('marathi') || lower.includes('मराठी')) ? 'mr' : 'en';
...
6. If drafting in Marathi for Maharashtra courts (e.g. Jalgaon, Amalner, Dhule), use standard court terminology relevant to the active document type (${context.templateTitle || 'Active Legal Document'}).`;
    ...
    async translateDocument(options) {
        const { documentBody, targetLanguage, apiKey } = options;
        if (!documentBody || !documentBody.trim())
            return '';
        const client = this.getClient(apiKey);
        if (!client) {
...
${documentBody}`;
    ...
    localTranslateHtml(html, targetLanguage) {
        if (!html)
            return '';
...
How would you like me to update your draft?`;
    ...
    async extractFactsFromNotes(rawNotes, apiKey) {
        const client = this.getClient(apiKey);
        if (client) {
            try {
...
${rawNotes}`;
        ...
        if (husbandMatch && husbandMatch[1].trim()) {
            ...
            if (!facts.deponentNewName)
                facts.deponentNewName = name;
        }
        ...
        // 12. Court / City
        if (text.includes('Amalner') || text.includes('अमळनेर'))
            facts.courtCity = 'अमळनेर';
        else if (text.includes('Jalgaon') || text.includes('जळगाव'))
            facts.courtCity = 'जळगाव';
        else if (text.includes('Dhule') || text.includes('धुळे'))
            facts.courtCity = 'धुळे';
        else if (text.includes('Pune') || text.includes('पुणे'))
            facts.courtCity = 'पुणे';
        else if (text.includes('Mumbai') || text.includes('मुंबई'))
            facts.courtCity = 'मुंबई';
        return {
            facts,
            summary: `Extracted ${Object.keys(facts).length} fields from client notes.`
        };
    }
    async transliterateText(text, customKey) {
        if (!text || !text.trim())
            return '';
...
Text to transliterate: "${text}"`;
    ...
    localPhoneticTransliterate(input) {
        if (!input)
            return '';
