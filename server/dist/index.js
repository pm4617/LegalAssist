...
const template_service_js_1 = require("./services/template.service.js");
const copilot_service_js_1 = require("./services/copilot.service.js");
...
app.get('/api/templates', (req, res) => {
    const templates = template_service_js_1.templateService.getAllTemplates();
    res.json(templates);
});
app.get('/api/templates/:id', (req, res) => {
    const template = template_service_js_1.templateService.getTemplate(req.params.id);
    if (!template) {
...
app.post('/api/templates', (req, res) => {
    try {
        ...
        const existing = template_service_js_1.templateService.getTemplate(template.id);
        ...
        const saved = template_service_js_1.templateService.saveTemplate({
            standardClauses: [],
            fields: [],
...
app.put('/api/templates/:id', (req, res) => {
    try {
        const existing = template_service_js_1.templateService.getTemplate(req.params.id);
        if (!existing)
            return res.status(404).json({ error: 'Template not found' });
        if (existing.isBuiltIn) {
            return res.status(403).json({ error: 'Cannot directly edit a built-in template. Clone it first.' });
        }
        const updated = template_service_js_1.templateService.saveTemplate({ ...req.body, id: req.params.id });
        res.json(updated);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.delete('/api/templates/:id', (req, res) => {
    try {
        template_service_js_1.templateService.deleteTemplate(req.params.id);
...
app.post('/api/templates/:id/clone', (req, res) => {
    try {
        ...
        if (!newId || !newTitle)
            return res.status(400).json({ error: 'newId and newTitle are required' });
        const cloned = template_service_js_1.templateService.cloneTemplate(req.params.id, newId, newTitle);
...
app.post('/api/documents/render', (req, res) => {
    ...
    const template = template_service_js_1.templateService.getTemplate(templateId);
    if (!template) {
        return res.status(404).json({ error: 'Template not found' });
    }
    const mergedText = template_service_js_1.templateService.mergeTemplate(template, facts || {});
    const compliance = template_service_js_1.templateService.auditCompliance(template, facts || {}, mergedText);
    res.json({
        text: mergedText,
        compliance
    });
...
app.post('/api/documents/audit', (req, res) => {
    ...
    const compliance = template_service_js_1.templateService.auditCompliance(template, facts || {}, draftText || '');
    res.json(compliance);
});
...
app.post('/api/documents/export/docx', async (req, res) => {
    try {
        ...
        const buffer = await export_service_js_1.exportService.generateDocx({
...
app.post('/api/copilot/chat', async (req, res) => {
    try {
        ...
        const response = await copilot_service_js_1.copilotService.processChat({
            message: message || '',
            context: context || {},
            apiKey
        });
...
app.post('/api/copilot/extract', async (req, res) => {
    try {
        ...
        const result = await copilot_service_js_1.copilotService.extractFactsFromNotes(rawNotes, apiKey);
        res.json(result);
    }
    catch (err) {
...
app.post('/api/copilot/transliterate', async (req, res) => {
    try {
        ...
        const transliterated = await copilot_service_js_1.copilotService.transliterateText(text, apiKey);
...
app.post('/api/copilot/translate', async (req, res) => {
    try {
        ...
        const geminiKey = apiKey || req.headers['x-gemini-api-key'] || process.env.GEMINI_API_KEY;
        if (!geminiKey) {
        ...
        const translated = await copilot_service_js_1.copilotService.translateDocument({
            documentBody,
...
app.all(['/api/copilot', '/api/copilot/*', '/api/copilot/agents/:agent/run', '/api/copilot/agent/:agent/run'], async (req, res) => {
    ...
    try {
        ...
        function buildClientContext(body) {
            ...
            if (!docBody || !title || !facts) {
                const checkItem = (item) => {
                    ...
                    if (!docBody && (val?.documentBody || val?.body))
                        docBody = val.documentBody || val.body;
                    if (!title && (val?.templateTitle || val?.title))
                        title = val.templateTitle || val.title;
                    if (!facts && (val?.party1Name || val?.courtCity || val?.clientFacts))
                        facts = val.clientFacts || val;
                };
                if (Array.isArray(cc))
                    cc.forEach(checkItem);
                if (Array.isArray(body?.readable))
                    body.readable.forEach(checkItem);
                if (Array.isArray(body?.messages)) {
                    body.messages.forEach((m) => {
                        if (m?.clientContext) {
                            if (Array.isArray(m.clientContext))
                                m.clientContext.forEach(checkItem);
                            else
                                checkItem(m.clientContext);
                        }
                    });
                }
            }
            ...
            for (let i = msgs.length - 1; i >= 0; i--) {
                ...
                if (typeof msg === 'object') {
                    ...
                    if (Array.isArray(msg.content)) {
                        const textObj = msg.content.find((c) => typeof c === 'string' || c?.type === 'text' || c?.text);
                    ...
                    if (Array.isArray(msg.parts)) {
                        const textPart = msg.parts.find((p) => typeof p === 'string' || p?.text);
        ...
        if (!geminiKey) {
            ...
            const reply = await copilot_service_js_1.copilotService.processChat({
        ...
        const cleanError = (0, copilot_service_js_1.formatGeminiErrorMessage)(err);
        emit('TEXT_MESSAGE_START', { message_id: msgId, messageId: msgId, id: msgId, role: 'assistant', specificationVersion: 'v1' });
